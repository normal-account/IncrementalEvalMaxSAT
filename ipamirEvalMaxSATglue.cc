...
public:
   ...
   // The ipamir interface overrides the weight for -lit or lit (see them as separate).
   // The structures for referenceCards should be made with that in mind and not use unsigned types

   /* The soft lits, independently of their model, are still in the hard clauses as you would expect.
    * Therefore, you can just manipulate the assumption in case of a model change for a given soft lit.
    *
    * If the model changes, then the first card is useless, and so are possibly all the other cards that are a result of it.
    * This means that we need to remove cards in some cases, but in other cases we can just fix them.
    * Let's start with the naive implementation of removing all cards no matter the situation, and improve if time allows.
    *
    * Removing a card implies to, recursively, check all its other contained cards, and for regular soft lits, assume em again.
    * When it comes to cost management, we'd want to substract from every card the minWeight, and for every reg soft lit
    * that is NOT assumed, its ogWeight from the cost. We'd also want to fix the _weight structure and possibly the model
    *
    * */
   void add_soft_lit(int32_t newLit, uint64_t w) {
       ...
       if (alreadyDeclared && !firstSolve) {
           maxSATnoAssumed->fixAllCards(std::abs(newLit), w);
           // Saves the updated weights into a vector so that the weights can be fixed according to the true ogWeights
           maxSATnoAssumed->setOGWeight(std::abs(newLit), w); // TODO : ogWeight should be fixed at the call of solve()
       }
       else
           maxSATnoAssumed->setVarSoft(std::abs(newLit), value, w);
   ...
   int32_t solve() {
       ...
       maxSATwithAssump->setUserAssumps(assumptions);
