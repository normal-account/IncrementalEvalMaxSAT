...
class CardIncremental_Lazy : public VirtualCard {
   ...
   struct TotTree {
        ...
        TotTree(const TotTree& other) {
            lazyVars = other.lazyVars;
            nof_input = other.nof_input;
            if (other.left != nullptr) {
                auto newLeft = *other.left;
                left = std::make_shared<TotTree>(newLeft);
            }

            if (other.right != nullptr) {
                auto newRight = *other.right;
                right = std::make_shared<TotTree>(newRight);
            }
        }
...
public:
    ...
    CardIncremental_Lazy( CardIncremental_Lazy &other );
